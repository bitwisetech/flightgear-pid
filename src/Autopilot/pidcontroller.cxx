...
void PIDController::update( bool firstTime, double dt ) 
{
    if( firstTime ) {
      ...
      // first time being enabled, seed 'last output' with current property tree value
      u_n_1 = get_output_value();
      if( _debug ) cout << "PID firstTime:" << subsystemId() << " " 
    ...
    if( Ts > SGLimitsd::min()) {
        ...
        if (_copy_pid_to_props) {
            // copy accumulated components of Process Output to props tree 
            fgSetFloat("autopilot/internal/pid/P-Term", accum_P);
            fgSetFloat("autopilot/internal/pid/I-Term", accum_I);
            fgSetFloat("autopilot/internal/pid/D-Term", accum_D);
				}		
        ...
        if (_copy_pid_to_props) {
            // copy accumulated components of Process Output to props tree 
            fgSetFloat("autopilot/internal/pid/PID-Out", accum_P);
        }    
        ...
        if ( _debug ) {
          cout << "PID: "<< subsystemId() << " "
            ...
            << " P :"   << std::fixed << std::showpos << std::setprecision( 3 ) 
              << accum_P
            << " I :"   << std::fixed << std::showpos << std::setprecision( 3 ) 
              << accum_I
            << " D :"   << std::fixed << std::showpos << std::setprecision( 3 ) 
              << accum_D
...
//   was wrong to change Yn and Un-1 gets seeded in firstTime after enabled 
